...
function isMergableExported(variable) {
    ...
    for (const def of variable.defs) {
        ...
        if ((MERGABLE_TYPES.has(def.node.type) &&
            def.node.parent?.type === experimental_utils_1.AST_NODE_TYPES.ExportNamedDeclaration) ||
            def.node.parent?.type === experimental_utils_1.AST_NODE_TYPES.ExportDefaultDeclaration) {
            return true;
        }
    }
    return false;
    ...
    function getFunctionDefinitions(variable) {
        ...
        variable.defs.forEach((def) => {
            ...
            if (def.type === experimental_utils_1.TSESLint.Scope.DefinitionType.Variable &&
                (def.node.init?.type === experimental_utils_1.AST_NODE_TYPES.FunctionExpression ||
                    def.node.init?.type === experimental_utils_1.AST_NODE_TYPES.ArrowFunctionExpression)) {
